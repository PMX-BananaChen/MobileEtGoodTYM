			...
			<view class="s-table__wrap">
				<view v-if="showHeader" class="s-table__header" :style="header_style">
					<slot name="header" :scopeParams="scopeParams" :columnWidthStyleList="columnWidthStyleList">
						<template v-for="(column, columnIndex) of columns">
							<view v-if="!column.hidden" :key="columnIndex" class="s-table__th"
								:class="thClassList[columnIndex]" :style="thStyleList[columnIndex]"
								@click.stop="onThClick(column, columnIndex)">
								<slot name="th" :scopeParams="scopeParams" :column="column" :columnIndex="columnIndex">
									<view>{{ column.label }}</view>
									<view class="th_select" v-if="showSelectIndex==columnIndex">
										<view v-for="(item,index) in column.selectList" :key="index"
											:class="{'selectActive':index==column.selectIndex}"
											@click="onSelectClick(columnIndex,index)">
											{{item}}
										</view>
									</view>
									<view class="s-table__sort-icon2" v-if="hasSelect(column)"></view>
									<view v-if="hasSort(column)" class="s-table__sort-wrap">
										<view v-for="order of ['asc', 'desc']" :key="order" class="s-table__sort-icon"
				...
				<view class="s-table__body">
					<scroll-view class="s-table__scroll-y" scroll-y>
						<view v-for="(row, rowIndex) of data" :key="rowIndex" class="s-table__tr"
							:class="'' + getRowClass(row, rowIndex)" :style="'' + getRowStyle(row, rowIndex)"
							@click="onTrClick(row, rowIndex)">
							<slot name="tr" :scopeParams="scopeParams" :row="row" :rowIndex="rowIndex"
								:columnWidthStyleList="columnWidthStyleList">
								<template v-for="(column, columnIndex) of columns">
									<view v-if="!column.hidden" :key="columnIndex" class="s-table__td"
										...
										<slot :scopeParams="scopeParams" :row="row" :rowIndex="rowIndex"
...
<script>
	...
	export default {
		...
		methods: {
			hasSelect(column) {
				return this.$isDef(column.selectList);
			},
			hasSort(column) {
			...
			onThClick(column, columnIndex) {
				...
				} else if (this.hasSelect(column)) {
					this.$emit('changeSelect', columnIndex);
				}
			},
			...
			onSelectClick(columnIndex,index) {
				this.$emit('onSelectClick',columnIndex, index);
			}
		},
	};
</script>
