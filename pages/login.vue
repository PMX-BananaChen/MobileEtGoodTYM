<template>
	<view>
		<div>
			<div class="gridCell">
				<van-cell-group>
					<van-cell title="申请" class="van-cellfont" is-link icon="chat-o"  @click.native="onClick(3,'/apply')" />
				</van-cell-group>
			...
			<div class="gridCell">
				<van-cell-group title="异常通报">
					<van-grid :column-num="3">
						<van-grid-item key="waitmcheckme" :icon="require('./images/confirms.png')"
							@click.native="onClick(3,'/mabnormalcheck')" text="待我确认">
						</van-grid-item>
						<van-grid-item key="waitmcheckmes" :icon="require('./images/abnormal.png')"
							@click.native="onClick(3,'/mabnormalchecks')" text="异常单">
						</van-grid-item>
					</van-grid>
				</van-cell-group>
			</div>
...
<script>
	...
	export default {
		...
		created() {
			...
			// 高彩會*/
			// uni.setStorageSync('UserId', '169971');
			// uni.setStorageSync('UserName', '曹國祥');
			// uni.setStorageSync('Sex', 'F');
			// uni.setStorageSync('Factory', 'IT');
		},
		methods: {
			setUser() {
			...
			if (code) {// 通过code获取 openId等用户信息
				...
				Conversion.getCodeApi('releaseGood')
			}
			},
			// 获取当前登录人名称
			setUserName(userId) {
			...
			onIndexChange(e) {
				let indexv = e.detail;
				uni.setStorageSync('TableIndex',  '/login');
				switch (indexv) {
					case 'apply':
						this.title = '申请'
