<template>
	<form ref="form" model="form">
		...
		<van-cell title="审核状态" v-model="titlesv" ></van-cell>
		<van-field v-model="form.returntype"></van-field>
		<van-field v-model="form.returntype"></van-field>
		<yy-qrcode :text="qrcodeText" v-if="isRresh"></yy-qrcode>
		<br/>
		<br/>
		<br/>
	...
	export default {
		...
		methods: {
			tabIndex(index) {
				...
				uni.setStorageSync('TableIndex',  '/waitcheckeddetail');
			...
			getdata() {
				...
				getReleaseGood(self.id).then(data => {
					...
					if (data.code === 200) {
						...
						self.isRresh=true;
						self.qrcodeText=self.form.carrierin;
