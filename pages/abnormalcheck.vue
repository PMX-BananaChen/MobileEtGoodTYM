<template>
	<form ref="form" model="form">
		...
		<div style="margin: 0px auto;" v-if="!showT">
			...
			<van-cell class="myCell" title="通報異常安檢站："    :value="form.sitename"/>
			<van-cell style="color:red" title="異常內容："  :value="form.errremark" />
			...
			<view v-if="goodcompanyv">
				<uni-table ref="tablecompany" border>
					<uni-tr>
						<uni-th width="6" align="center">物品名稱</uni-th>
	...
	export default {
		...
		methods: {
			...
			tabIndex(index) {
				...
				uni.setStorageSync('TableIndex',  '/abnormalcheck');
				console.log(val);
			...
			submit(ref) {
				...
				if (ref == 'b') {
					...
					this.flow.result = '1';
					this.flow.resultstation='';
				}
				this.flow.formno=this.form.formno;
				this.flow.sitename=this.form.sitename;
				updateFlow(this.flow).then(response => {
