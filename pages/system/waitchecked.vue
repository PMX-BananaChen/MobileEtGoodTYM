<template>
	<form ref="form" model="form">
		<!-- 无数据显示 -->
		<div><van-nav-bar left-arrow @click-left="backHome" />
		</div>
...
<script>
	...
	export default {
		...
		created() { //查询数据
			...
			listReleaseGood2(this.form).then(response => {
		...
		methods: {
			tabIndex(index) {
				index = Number(index)
				this.active = index;
				// this.$store.commit('setTableIndex', '/waitchecked');
				uni.setStorageSync('TableIndex',  '/waitchecked');
				// 记录当前tab页
			...
			goDetailOpin(value) {
				...
				let val = "/waitcheckeddetail" + "?id=" + value
