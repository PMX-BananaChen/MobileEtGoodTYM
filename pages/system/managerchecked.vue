		...
		created() { //查询数据
			...
			listReleaseGood(this.form).then(response => {
				if (response.code === 200) {
					...
					// this.$nextTick(() => {
					// 	this.showT = this.opinionsList.length === 0;
					// });
				} else {
			...
			// this.$nextTick(() => {
			// 	this.show =
			// 		(this.opinionsList.length === 0 || this.opinShow);
			// });
		},
		onLoad() {},
		methods: {
			tabIndex(index) {
				...
				this.active = index;
				// 记录当前tab页
				// this.$store.commit('setTableIndex', '/managerchecked');
				uni.setStorageSync('TableIndex',  '/managerchecked');
