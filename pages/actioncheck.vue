		...
		<div style="margin: 0px auto;" v-if="!showT">
			...
			<view v-if="goodprivatev">
				<uni-table ref="tableprivate">
					<uni-tr>
						...
						<uni-th width="100px;" align="center">物品名稱<span style="color:red">*</span></uni-th>
						<uni-th width="40px;" align="center">單位<span style="color:red">*</span></uni-th>
					...
					<uni-tr v-for="(item, index) in form.privates" :key="index">
						...
						<uni-td align="center">
							...
							<image v-for="(url, index) in imageUrls[index]" :key="index" :src="url"
								@click="previewImage(index)" mode="widthFix" style="width: 100%;"></image>
						</uni-td>
					</uni-tr>
				</uni-table>
			</view>
			...
			<view v-if="goodcompanyv">
				<uni-table ref="tablecompany" border>
					<uni-tr>
						<uni-th width="100" align="center">物品名稱</uni-th>
						<uni-th width="40" align="center">單位</uni-th>
						<uni-th width="40" align="center">數量</uni-th>
						<uni-th width="120" align="center">放行原因</uni-th>
			...
			<van-field v-model="form.backremark" v-if="!butonShow" placeholder="请输入退回原因"
				:error-message="errorMsg.resulta" type="textarea" @change="query" autosize />
	...
	export default {
		...
		methods: {
			...
			tabIndex(index) {
				...
				this.active = index;
				// 记录当前tab页
				uni.setStorageSync('TableIndex', '/actioncheck');
			...
			submit(ref) {
				...
				// let managernos = this.opinionsList.checkno.replace(/\b(0+)/gi, "");
				// this.form.checkno = managernos;
			...
			getdata() {
				...
				getReleaseGood(self.id).then(data => {
					if (data.code === 200) {
						self.form = data.data;
						switch(self.form.result.replace(' ', '')) //已经审核过
						{  
							case "1":
								self.showT = true;
